...
import { useDailyRoundData } from '@/src/hooks/useDailyRoundData';
                            ...
                            <div key={animal.id} className={`bg-white border-2 rounded-2xl p-3 md:p-4 flex items-center gap-3 md:gap-6 transition-all ${isDone ? 'border-emerald-100 shadow-sm' : (state.isAlive === false || state.securityIssue) ? 'border-rose-100 bg-rose-50' : 'border-slate-200'}`}>
                                <div className="flex items-center gap-4 flex-1 min-w-0">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-black text-slate-800 text-sm md:text-base truncate uppercase tracking-tight">{animal.name}</h3>
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{animal.location}</p>
                ...
                <div className="max-w-4xl mx-auto space-y-4">
                    <div>
                        <div className="flex justify-between items-end mb-1.5">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{activeTab} Section Progress</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        >
                            ...
                            {isPastRound ? 'Signed Off' : `Verify & Sign Off ${activeTab}`}
                        </button>
                    </div>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 border-2 border-slate-200">
                        ...
                        <div className="p-8 space-y-6">
                            <p className="text-sm font-medium text-slate-600 leading-relaxed">
                                You are flagging <strong>{categoryAnimals.find(a => a.id === reportAnimalId)?.name}</strong>. 
