import { v4 as uuidv4 } from 'uuid';
import { db } from '@/src/db';
import { createRecord, updateRecord, deleteRecord } from '@/src/services/dataService';
import { HolidayRequest } from '@/types';
import { useAuthStore } from '@/src/store/authStore';

export const addHolidayRequest = async (request: Omit<HolidayRequest, 'id' | 'created_at' | 'updated_at' | 'created_by' | 'last_modified_by'>) => {
  const currentUser = useAuthStore.getState().profile;
  if (!currentUser) throw new Error('User not authenticated');
  const newRequest: HolidayRequest = {
    ...request,
    id: uuidv4(),
    created_at: new Date(),
    updated_at: new Date(),
    created_by: currentUser.id,
    last_modified_by: currentUser.id,
  };
  await createRecord('holiday_requests', db.holiday_requests, newRequest);
};

export const updateHolidayRequest = async (updates: Partial<HolidayRequest>) => {
  const currentUser = useAuthStore.getState().profile;
  if (!currentUser) throw new Error('User not authenticated');
  await updateRecord('holiday_requests', db.holiday_requests, updates.id!, { ...updates, updated_at: new Date(), last_modified_by: currentUser.id });
};

export const deleteHolidayRequest = async (id: string) => {
  await deleteRecord('holiday_requests', db.holiday_requests, id);
};
